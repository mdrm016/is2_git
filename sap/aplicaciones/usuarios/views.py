...
def administrarUsuarios(request):
    ...
    if 'busqueda' in request.GET:
        ...
        logger.info("Busqueda de usuario con patron %s" % busqueda)
        if not busqueda:
            error = True
            ...
            if (not usuarioname) & (not usuarionombre) & (not usuarioemail) & (not usuariotelefono) & (not usuariodireccion) & (
                    not usuarioespecialidad) & (not usuarioobservaciones):
                error = True
                template_name = './Usuarios/usuarios.html'
                return render(request, template_name, {'error': error})
            else:
    ...
    logger.info("Listado de usuarios, hecho por %s" % request.user.username)
    template_name = './Usuarios/usuarios.html'
...
def usuarionuevo(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            logger.info("Creacion de usuario %s, hecho por %s" % (useri.username, request.user.username))
    ...
    if solicitante.has_perm('usuarios.modificar_usuarios') or solicitante.id == id_usuario:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                profile.save()
                logger.info("Modificacion de datos de usuario %s, hecho por %s" % (usuario.username, request.user.username))
            ...
            data = {'Nombre_de_Usuario': usuario.username, 'Contrasenha': '', 'Nueva_contrasenha': '',
                    ...
                    'Telefono': perfil.telefono, 'Direccion': perfil.direccion, 'Especialidad': perfil.especialidad,
...
def consultarUsuario(request, id_usuario):
    ...
    perfil = usuario.get_profile()
    logger.info("Consulta de datos dee usuario %s, hecho por %s" % (usuario.username, request.user.username))
...
def usuario_eliminar(request, id_usuario):
    ...
    if id_usuario != '1':
        ...
        if proyecto:
            ...
            userDelLogic.save()
            logger.info("Eliminacion logica de usuario %s, hecho por %s" % (userDelLogic.username, request.user.username))
...
def buscar_usuario(request):
    ...
    if 'busqueda' in request.GET:
        busqueda = request.GET['busqueda']
    logger.info("Busqueda de usuario con patron %s, hecho por %s" % (busqueda, request.user.username))
    if not busqueda:
        error = True
    else:
